<template>
  <v-data-table :headers="headers"
    ...
    <template v-slot:top>
      <v-toolbar flat>
        <v-toolbar-title class="blue-text big-bold">ACTED MAKER REQUESTS</v-toolbar-title>
        <v-spacer></v-spacer>
...
<script>
  ...
  export default {
    ...
    created() {
      this.getActedMakerRequests();
    },
    methods: {
      async getActedMakerRequests() {
        this.loading = true;
        try {
          const response = await this.$axios.get(`${this.$config.restUrl}/api/serviceRequest/GetActedMakerRequest`);
          ...
          console.error('Error fetching acted maker requests:', error);
        } finally {
          this.loading = false;
        }
      },
