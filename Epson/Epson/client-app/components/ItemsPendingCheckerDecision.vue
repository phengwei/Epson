...
<script>
  ...
  export default {
    name: "ItemsPendingCheckerDecision",
    data() {
      return {
        requests: [],
        totalItems: 0,
    ...
    methods: {
      getPendingCheckerItems() {
        ...
        this.$axios
          .get(`${this.$config.restUrl}/api/serviceRequest/GetPendingCheckerItems`, { params })
          .then((response) => {
          ...
          .catch((error) => {
            this.loading = false;
            console.error("Error fetching checker items:", error);
          });
      },
