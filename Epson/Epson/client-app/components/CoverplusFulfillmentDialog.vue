<template>
  <v-dialog v-model="localDialogCoverplusFulfillment" max-width="800px">
    <v-card>
      <v-card-title>
      ...
      <v-card-text>
        ...
        <div class="form-group">
          ...
          <input v-model="localEditedItem.distyPrice" class="border-input" label="Disty Price"></input>
        </div>
        <div class="form-group">
          <label>Dealer Price</label>
          <input v-model="localEditedItem.dealerPrice" class="border-input" label="Dealer Price"></input>
        </div>
        <div class="form-group">
          <label>End User Price</label>
          <input v-model="localEditedItem.endUserPrice" class="border-input" label="End User Price"></input>
        </div>
        <div class="form-group">
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        localDialogCoverplusFulfillment: this.dialogCoverplusFulfillment,
        localEditedItem: {},

      };
    },
    watch: {
      dialogCoverplusFulfillment(newVal, oldVal) {
        this.localDialogCoverplusFulfillment = newVal;
      },
      localDialogCoverplusFulfillment(newVal, oldVal) {
        this.$emit('update:dialogCoverplusFulfillment', newVal);
      },
      editedItem: {
        handler(newValue) {
    ...
    methods: {
        ...
        if (this.localEditedItem.fulfilledPrice !== null && this.localEditedItem.fulfilledPrice > 0) {
          Swal.fire({
            ...
            if (result.isConfirmed) {
              this.$axios.post(`${this.$config.restUrl}/api/request/fulfillrequest?id=${this.localEditedItem.id}&sla=${this.localEditedItem.sla}&productId=${this.localEditedItem.productId}&fulfilledPrice=${this.localEditedItem.fulfilledPrice}&remarks=${this.localEditedItem.remarks}&distyPrice=${this.localEditedItem.distyPrice}&dealerPrice=${this.localEditedItem.dealerPrice}&endUserPrice=${this.localEditedItem.endUserPrice}`)
                .then(response => {
                  location.reload();
                }).catch(error => {
          ...
          this.$swal('Error', 'Please enter a valid dealer price!', 'error');
        }
      },
      ...
      close() {
        this.localDialogCoverplusFulfillment = false;
